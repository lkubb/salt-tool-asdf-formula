...
We will see a quick setup to configure the ``tool_asdf`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_asdf`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_asdf`` formulas under ``/srv/salt/tool_asdf/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_asdf/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_asdf/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_asdf/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_asdf/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_asdf`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_asdf`` formulas under ``/srv/salt/tool_asdf/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_asdf/parameters/roles
...
We create a configuration for the role ``tool_asdf/server`` in ``/srv/salt/tool_asdf/parameters/roles/tool_asdf/server.yaml``:
...
We create another configuration for the role ``tool_asdf/client`` in ``/srv/salt/tool_asdf/parameters/roles/tool_asdf/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_asdf/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_asdf/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_asdf/parameters/dns:domain/  '/srv/salt/tool_asdf/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-asdf-as-json:
      test.nop:
        - name: {{ asdf | json }}
...
.. code-block:: sls
    ...
    asdf-config-file-file-managed:
      file.managed:
        - name: {{ asdf.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='asdf-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ asdf.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``asdf`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ asdf['winner'] }}
